...
`),l=(s.bullets??[]).map(u=>`<li>${B(u)}</li>`).join(`
        `);return`
<section class="bb-section bb-region" data-section="region" data-region="${t}" id="zone-${t}">
  ...
  <p class="bb-region__progress-label">${B(o)}</p>
...
`);const r=document.getElementById("bb-header-logo");r&&(r.innerHTML=P),xe(e),_e(e),$e(e)}function xe(e){const t=e.querySelectorAll(".bb-section"),s=new IntersectionObserver(n=>{for(const a of n)a.isIntersecting&&(a.target.classList.add("is-visible"),s.unobserve(a.target))},{root:null,rootMargin:"0px 0px -8% 0px",threshold:.08});t.forEach(n=>s.observe(n))}function _e(e){const t=e.querySelector("[data-neuro-score]");if(!t)return;const s=parseFloat(t.textContent||"0"),n=1200,a=performance.now(),r=o=>{const i=Math.min(1,(o-a)/n),l=1-(1-i)**3;t.textContent=(s*l).toFixed(1),i<1&&requestAnimationFrame(r)};requestAnimationFrame(r)}function $e(e){const t=e.querySelectorAll("[data-bar-value]"),s=new IntersectionObserver(n=>{for(const a of n){if(!a.isIntersecting)continue;const r=a.target,o=parseFloat(r.getAttribute("data-bar-value")||"0"),i=r.querySelector(".pdf-bar-gradient");i&&(i.style.clipPath="inset(0 100% 0 0)",requestAnimationFrame(()=>{i.style.clipPath=`inset(0 ${Math.max(0,100-o).toFixed(2)}% 0 0)`})),s.unobserve(r)}},{threshold:.2});t.forEach(n=>s.observe(n))}function A(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function we(e,t){if(!e)return"—";try{return new Date(e).toLocaleString(t==="en"?"en-GB":"ru-RU",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return e}}async function Te(e,t,s){var l,u;const n=x(s.lang),a=document.createElement("section");a.className="bb-section is-visible bb-history",a.innerHTML=`
  ...
  `,(n=e.querySelector("#bb-premium-buy"))==null||n.addEventListener("click",()=>{C(),t.tributeUrl&&me(t.tributeUrl)})}function m(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function Ne(e,t,s,{onProfile:n}={}){const a=x(s.lang),r={variant:s.testVariant||"development",questions:[],answers:{},step:0},o=document.createElement("section");o.className="bb-section is-visible bb-test",e.replaceChildren(o);async function i(){o.innerHTML=`<p class="text-cyan-200/80 text-sm">${m(a.loading)}</p>`;try{const c=await le(t,r.variant);r.questions=c.questions??[],r.step=0,r.answers={},u()}catch{o.innerHTML=`<p class="bb-error">${m(a.errorLoad)}</p>`}}function l(){o.innerHTML=`
    ...
    `,o.querySelectorAll("[data-variant]").forEach(c=>{c.addEventListener("click",()=>{C(),r.variant=c.getAttribute("data-variant")||"development",i()})})}function u(){const c=r.questions[r.step];if(!c){l();return}const b=r.questions.length,v=(r.step+1)/b*100;o.innerHTML=`
      <div class="bb-test-progress mb-4">
        <div class="flex justify-between text-xs text-cyan-200/80 mb-1">
          <span>${m(a.questionProgress(r.step+1,b))}</span>
  ...
  `,t.querySelectorAll(".bb-nav__btn").forEach(n=>{n.addEventListener("click",()=>{C(),w(n.dataset.route||"map")})})}async function O(e){var s;const t=document.getElementById("bb-root");if(!(!t||!f)){if(Se(e.name),e.name==="premium"){p||(p=await M(f)),Le(t,p);return}if(e.name==="test"){p||(p=await M(f)),await Ne(t,f,p,{onProfile:n=>{p=n}});return}if(e.name==="history"){p||(p=await M(f)),await Te(t,f,p);return}if(!p){const n=x(f.lang);t.innerHTML=`
      <div class="bb-loading">
        <div class="bb-loading__pulse"></div>
        <p class="text-sm text-cyan-200/80">${n.loading}</p>
      </div>`;try{p=await M(f),(s=f.user)!=null&&s.first_name&&!p.userDisplayName&&(p.userDisplayName=[f.user.first_name,f.user.last_name].filter(Boolean).join(" "))}catch(a){const r=(a==null?void 0:a.status)===403?n.notRegistered:(a==null?void 0:a.status)===401?n.authError:n.errorLoad;t.innerHTML=`<p class="bb-error">${r}</p>`;return}}if(!p.hasMap&&e.name==="map"){w("test");return}ye(t,p),Ce(p)}}async function ke(e){f=e,p=null,document.body.classList.add("bb-app--telegram");const t=document.getElementById("bb-root");if(!t)return;t.classList.add("bb-root--spa");const s=x(e.lang),n=document.getElementById("bb-header"),a=document.getElementById("bb-header-title"),r=document.getElementById("bb-header-logo");a&&(a.textContent=s.appHeaderTitle),r&&(r.innerHTML=P),n&&(n.hidden=!1,n.classList.add("is-visible")),Ee(e.lang),be(i=>{O(i).catch(()=>{})}),pe();const o=G();window.location.hash?await O(o):w("map")}const R="/webapp/assets/full-glowing-brain-Cl127Rfm.png";function d(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Me(){return(navigator.language||"ru").toLowerCase().startsWith("en")?"en":"ru"}async function Be(){const e=Me(),t=x(e);document.documentElement.lang=e;const s=document.getElementById("bb-header"),n=document.getElementById("bb-nav");s&&(s.hidden=!0),n&&(n.hidden=!0);const a=document.getElementById("bb-root");if(!a)return;a.className="bb-root bb-root--landing",a.innerHTML=`
    <div class="bb-loading">
      <div class="bb-loading__pulse"></div>
  ...
  `,a.querySelectorAll(".bb-landing-about__photo").forEach(l=>{const u=l.getAttribute("data-fallback-src");u&&l.addEventListener("error",()=>{l.removeAttribute("data-fallback-src"),l.src=u})}),a.querySelectorAll('a[href^="#"]').forEach(l=>{l.addEventListener("click",u=>{var b;const h=(b=l.getAttribute("href"))==null?void 0:b.slice(1);if(!h)return;const c=document.getElementById(h);c&&(u.preventDefault(),c.scrollIntoView({behavior:"smooth",block:"start"}))})})}function Ae(){var s;const e=(s=window.Telegram)==null?void 0:s.WebApp;return e?(e.initData||"").trim().length>0:!1}const{initData:Pe,user:qe,lang:He}=ge();Ae()?ke({initData:Pe,user:qe,lang:He}):Be();
